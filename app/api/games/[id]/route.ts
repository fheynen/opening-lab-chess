...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const review = await db.select().from(gameReviews).where(and(eq(gameReviews.gameId, id), eq(gameReviews.userId, auth.user.id))).get();
  const moves = review ? await db.select().from(moveReviews).where(and(eq(moveReviews.gameId, id), eq(moveReviews.userId, auth.user.id))).orderBy(moveReviews.ply) : [];
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await (await getDb()).delete(games).where(and(eq(games.id, id), eq(games.userId, auth.user.id)));
