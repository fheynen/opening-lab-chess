...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.insert(gameReviews).values({ gameId: id, userId: auth.user.email, engineVersion: String(body.engineVersion || "Stockfish 18 lite"), depth: Math.max(1, Math.min(30, Number(body.depth || 12))), accuracy: Math.round(Number(body.accuracy)), summaryJson: JSON.stringify(body.summary || {}), strengthsJson: JSON.stringify(body.strengths || []), improvementsJson: JSON.stringify(body.improvements || []), createdAt: Date.now() });
  if (body.moves.length) await db.insert(moveReviews).values(body.moves.map((move) => ({ id: crypto.randomUUID(), gameId: id, userId: auth.user!.email, ply: Math.round(move.ply), moveNumber: Math.round(move.moveNumber), side: String(move.side), san: String(move.san), uci: String(move.uci), classification: String(move.classification), cpLoss: Math.max(0, Math.round(move.cpLoss)), winDrop: Math.max(0, Math.round(move.winDrop)), accuracy: Math.max(0, Math.min(100, Math.round(move.accuracy))), phase: String(move.phase), fenBefore: String(move.fenBefore), bestMoveUci: move.bestMoveUci || null, bestMoveSan: move.bestMoveSan || null, motifsJson: JSON.stringify(move.motifs || []), pvJson: JSON.stringify((move.pv || []).slice(0, 6)) })));
