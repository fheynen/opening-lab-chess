...
export async function GET() {
  ...
  const attempts = await (await getDb()).select().from(puzzleAttempts).where(eq(puzzleAttempts.userId, auth.user.email)).orderBy(desc(puzzleAttempts.attemptedAt)).limit(200);
...
export async function POST(request: Request) {
  ...
  await (await getDb()).insert(puzzleAttempts).values({ id: crypto.randomUUID(), userId: auth.user.email, puzzleId: body.puzzleId, pathId: body.pathId, solved: Boolean(body.solved), usedHint: Boolean(body.usedHint), attemptedAt: Date.now() });
