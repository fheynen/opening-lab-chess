...
export async function GET() {
  ...
  const rows = await (await getDb()).select({ openingId: openingProgress.openingId }).from(openingProgress).where(eq(openingProgress.userId, auth.user.email));
...
export async function POST(request: Request) {
  ...
  for (const openingId of completed) await db.insert(openingProgress).values({ userId: auth.user.email, openingId, completedAt: now }).onConflictDoNothing();
  await db.update(profiles).set({ localProgressMigrated: true, updatedAt: now }).where(eq(profiles.userId, auth.user.email));
